...
class ProjectEnhancement(models.Model):
    ...
    allocated_hours = fields.Float(string="Allocated Hours", compute="_compute_allocated_hours", store=True, tracking=True)
...
# You may also want to update your date constraint to use the correct fields:
    ...
    department_manager_id = fields.Many2one(
        ...
        compute="_compute_department_manager", store=True, readonly=True
    )
    ...
    # def write(self, vals):
    #     # 1. Capture the existing role state before updating the database cache
    #     old_roles = {}
    #     for rec in self:
    #         old_roles[rec.id] = {
    #             'team_lead': rec.team_lead_id.id,
    #             'dept_manager': rec.department_manager_id.id,
    #             'pm': rec.project_manager.id,
    #         }
    #
    #     # 2. Run the master write method execution
    #     res = super(ProjectEnhancement, self).write(vals)
    #
    #     # 3. Assess variations to send assignment alerts
    #     for rec in self:
    #         old = old_roles.get(rec.id, {'team_lead': False, 'dept_manager': False, 'pm': False})
    #
    #         # Check Team Lead Assignment
    #         if 'team_lead_id' in vals or rec.team_lead_id.id != old['team_lead']:
    #             if rec.team_lead_id:
    #                 rec._send_role_assignment_notification('team_lead', rec.team_lead_id)
    #
    #         # Check Department Manager Assignment (handles computed field transitions)
    #         if 'department_id' in vals or 'department_manager_id' in vals or rec.department_manager_id.id != old[
    #             'dept_manager']:
    #             if rec.department_manager_id:
    #                 rec._send_role_assignment_notification('department_manager', rec.department_manager_id)
    #
    #         # Check Project Manager Assignment
    #         if 'project_manager' in vals or rec.project_manager.id != old['pm']:
    #             if rec.project_manager:
    #                 rec._send_role_assignment_notification('project_manager', rec.project_manager)
    #
    #     return res
    def write(self, vals):
        # Capture the existing values BEFORE the write
        old_values = {}
        for rec in self:
            old_values[rec.id] = {
                'project_manager': rec.project_manager.id,
                'department_manager_id': rec.department_manager_id.id,
                'team_lead_id': rec.team_lead_id.id,
            }
        ...
        for rec in self:
            ...
            if 'project_manager' in vals or rec.project_manager.id != old.get('project_manager'):
                if rec.project_manager:
                    _logger.info(f"Sending project manager notification for {rec.name} to {rec.project_manager.name}")
                    rec._send_role_assignment_notification('project_manager', rec.project_manager)

            # Check if Department Manager changed
            if 'department_manager_id' in vals or rec.department_manager_id.id != old.get('department_manager_id'):
                if rec.department_manager_id:
                    _logger.info(
                        f"Sending department manager notification for {rec.name} to {rec.department_manager_id.name}")
                    rec._send_role_assignment_notification('department_manager', rec.department_manager_id)

            # Check if Team Lead changed
            if 'team_lead_id' in vals or rec.team_lead_id.id != old.get('team_lead_id'):
                if rec.team_lead_id:
                    _logger.info(f"Sending team lead notification for {rec.name} to {rec.team_lead_id.name}")
                    rec._send_role_assignment_notification('team_lead', rec.team_lead_id)

        return res


    # new modification today monday 17th of nov
    # @api.model
    # def create(self, vals):
    #     project = super().create(vals)
    #
    #     # This will ONLY trigger when project is auto-created from a Sales Order
    #     if project.sale_order_id:
    #         so = project.sale_order_id
    #
    #         # 1. Use custom title if user typed one
    #         if so.project_title and so.project_title.strip():
    #             base = so.project_title.strip()
    #         else:
    #             # 2. Fallback to SO number + client name
    #             client = so.partner_id.name or ""
    #             base = so.name + (f" - {client}" if client else "")
    #
    #         # 3. Generate PROJ/2025/001, PROJ/2025/002, etc.
    #         proj_code = self.env['ir.sequence'].next_by_code('project.auto.code') or 'PROJ/2025/001'
    #
    #         # Final name
    #         project.name = f"{base} / {proj_code}"
    #
    #         # Keep analytic account clean too
    ...
    activity_ids = fields.One2many('mail.activity', 'res_id', domain=lambda self: [('res_model', '=', self._name)], string='Activities', auto_join=True)
    ...
    can_edit_department_manager = fields.Boolean(string='Can Edit Department Manager', compute='_compute_role_edit_rights')
    ...
    def _compute_department_manager(self):
        for rec in self:
            rec.department_manager_id = rec.department_id.manager_id if rec.department_id else False
    ...
    #         rec._send_role_acceptance_email(role)
    def accept_role(self, role):
        for rec in self:
    ...
    #
    #     for rec in self:
    #         assigner = rec.write_uid or self.env.user
    #         notification_msg = f"{role.replace('_', ' ').title()} role for {rec.name} has been declined. Reason: {reason}"
    #         rec.message_notify(body=notification_msg, partner_ids=[assigner.partner_id.id])
    #         rec._send_role_decline_email(role, reason)
    #
    #         if role == 'project_manager':
    ...
    #         _logger.warning(f"Skipped assignment alert for {employee.name}: No email address found.")
    #         return False
    #
    ...
    #         <p>You have been assigned as the <strong>{role_title}</strong> for the project <strong>{self.name}</strong>.</p>
    #         <p>Please log in to your dashboard to review allocation rules and safely accept or decline this role assignment.</p>
    #         <p style="margin-top: 20px;">
    #             <a href="{self._get_base_url()}/web#id={self.id}&model=project.project&view_type=form"
    ...
    def _send_role_assignment_notification(self, role_name, employee_record):
        ...
        body_html = f"""
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px;">
                ...
                <p style="font-size: 12px; color: #666; margin-top: 20px;">
                    <em>Please log into Odoo to accept or decline this role. You will find Accept/Decline buttons on the project form.</em>
                </p>
            </div>
        """
        ...
        return True
    # def _send_role_assignment_notification(self, role_name, employee_record):
    #     """Send notification to assigned person to accept/decline the role"""
    #     self.ensure_one()
    #     employee = employee_record.sudo()
    #
    #     # Set status to pending if not already accepted
    #     if role_name == 'team_lead' and self.team_lead_status != 'accepted':
    #         self.team_lead_status = 'pending'
    #     elif role_name == 'department_manager' and self.department_manager_status != 'accepted':
    #         self.department_manager_status = 'pending'
    ...
    #
    #     if not employee.work_email:
    #         _logger.warning(f"Cannot send email to {employee.name}: No work email found")
    #         # Still try to create activity
    #
    #     role_title = role_name.replace('_', ' ').title()
    #     subject = f"Action Required: Assigned as {role_title} for '{self.name}'"
    #
    #     # Create accept/decline URLs
    #     base_url = self._get_base_url()
    #     accept_url = f"{base_url}/web#id={self.id}&model=project.project&view_type=form&action=accept"
    #
    #     body_html = f"""
    #         <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px;">
    #             <h2 style="color: #7C5BBA;">Role Assignment Notification</h2>
    ...
    #             <p>Please review the project requirements and confirm your acceptance or decline of this role.</p>
    #             <div style="margin: 25px 0;">
    #                 <a href="{accept_url}" style="background-color: #28a745; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px; margin-right: 10px; display: inline-block;">
    #                     ✅ Accept Role
    #                 </a>
    #                 <span style="color: #999;">or</span>
    #                 <a href="{base_url}/web#id={self.id}&model=project.project&view_type=form" style="background-color: #dc3545; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px; margin-left: 10px; display: inline-block;">
    #                     ❌ Decline Role
    #                 </a>
    #             </div>
    #             <p style="font-size: 12px; color: #666; margin-top: 20px;">
    #                 <em>Note: You can also respond by opening the project record and clicking the Accept/Decline buttons.</em>
    #             </p>
    #         </div>
    #     """
    #
    ...
    #             })
    #             mail.send()
    #             _logger.info(f"Email sent to {employee.work_email} for role {role_name}")
    #         except Exception as e:
    ...
    def create(self, vals):
        ...
        if project.sale_order_id and project.sale_order_id.project_document_link:
            ...
            _logger.info(f"Synced Teams link from SO {project.sale_order_id.name} to new Project {project.name}")

        # Set initial role statuses
        role_status_map = {
            'project_manager': 'project_manager_status',
            'team_lead_id': 'team_lead_status',
            'department_manager_id': 'department_manager_status',
        }
        for role_field, status_field in role_status_map.items():
            if vals.get(role_field) and not getattr(project, status_field):
                setattr(project, status_field, 'pending')

        # Log project creation
        self.env['project.change.log'].sudo().create({
            'project_id': project.id,
            'field_name': 'Project Creation',
            'old_value': '',
            'new_value': f"Project '{project.name}' was created",
            'user_id': self.env.uid,
            'name': f"Change Log: Project Created - {project.name}",
            'change_date': fields.Datetime.now(),
        })

        # Log initial role assignments
        for role_field in role_status_map.keys():
            if vals.get(role_field):
                employee = self.env['hr.employee'].browse(vals[role_field])
                if employee.exists():
                    self.env['project.change.log'].sudo().create({
                        'project_id': project.id,
                        'field_name': role_field,
                        'old_value': 'None',
                        'new_value': employee.name,
                        'user_id': self.env.uid,
                        'name': f"Change Log: {role_field.replace('_', ' ').title()} - {project.name}",
                        'change_date': fields.Datetime.now(),
                    })
                    project.message_post(
                        body=f"{role_field.replace('_', ' ').title()} set to {employee.name}",
                        message_type="notification",
                        subtype_xmlid="mail.mt_comment"
                    )
        ...
        for role_field in role_status_map.keys():
            if getattr(project, role_field):
                project._send_role_assignment_email(project, role_field)
    ...
    def write(self, vals):
        ...
        if 'department_id' in vals_copy:
            department = self.env['hr.department'].browse(vals_copy['department_id'])
            vals_copy['department_manager_id'] = department.manager_id.id if department.manager_id else False

        for project in self:
            local_vals = vals_copy.copy()

            # Reset statuses if roles changed
            for role_field, status_field in role_fields.items():
                if role_field in local_vals:
                    current_role = getattr(project, role_field)
                    current_id = current_role.id if isinstance(current_role, models.BaseModel) and len(current_role) == 1 else current_role.ids if hasattr(current_role, 'ids') else current_role
                    new_id = local_vals[role_field].id if isinstance(local_vals[role_field], models.BaseModel) else local_vals[role_field]
                    if current_id != new_id:
                        local_vals[status_field] = 'pending'

            # Track changes
            for field, new_val in local_vals.items():
                if field not in project._fields:
                    continue

                field_info = project._fields[field]
                old_val = getattr(project, field)

                if isinstance(field_info, fields.Many2one):
                    old_id = old_val.id if old_val and len(old_val) == 1 else False
                    new_id = new_val.id if isinstance(new_val, models.BaseModel) else new_val
                    if old_id != new_id:
                        old_label = old_val.name_get()[0][1] if old_val and old_val.exists() else ''
                        new_rec = self.env[field_info.comodel_name].browse(new_id) if new_id else None
                        new_label = new_rec.name_get()[0][1] if new_rec and new_rec.exists() else ''
                        changes.append({
                            'project_id': project.id,
                            'field_name': field,
                            'old_value': old_label,
                            'new_value': new_label,
                            'user_id': self.env.uid,
                            'name': f"Changed {field.replace('_', ' ').title()}",
                            'change_date': fields.Datetime.now(),
                        })

                elif isinstance(field_info, fields.Many2many):
                    old_ids = set(old_val.ids)
                    new_ids = set(self._extract_ids(new_val))
                    if old_ids != new_ids:
                        changes.append({
                            'project_id': project.id,
                            'field_name': field,
                            'old_value': ', '.join(old_val.mapped('name')),
                            'new_value': ', '.join(self.env[field_info.comodel_name].browse(list(new_ids)).mapped('name')),
                            'user_id': self.env.uid,
                            'name': f"Changed {field.replace('_', ' ').title()}",
                            'change_date': fields.Datetime.now(),
                        })
        ...
        result = super().write(vals_copy)

        for project in self:
            if changes:
                self.env['project.change.log'].sudo().create(changes)

            if hasattr(project, '_update_status_fields'):
                project._update_status_fields()

            for role_field in role_fields.keys():
                if role_field in vals_copy and hasattr(project, '_send_role_assignment_email'):
                    project._send_role_assignment_email(project, role_field)

            if 'expense_ids' in vals_copy and hasattr(project, '_check_budget_alert'):
                project._check_budget_alert()

        return result
        for record in self:
            record.total_project = count
